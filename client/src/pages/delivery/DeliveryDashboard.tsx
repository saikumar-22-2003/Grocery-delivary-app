...
export default function DeliveryDashboard() {
    ...
    const fetchOrders = async () => {
        setLoading(true);
        try {
            const { data } = await axios.get(`${API_URL}/delivery/my_deliveries?status=${tab}`,getAuthHeaders())
    ...
    // send location for every 10s for active deliveries
    useEffect(()=>{
    ...
    const handleUpdateStatus = async (orderId: string, status: string) => {
        try {
           await axios.put(`${API_URL}/delivery/my_deliveries/${orderId}/status`,
    ...
    const handleComplete = async () => {
        ...
        setSubmitting(true);
        try {
             await axios.put(`${API_URL}/delivery/my_deliveries/${otpModal}/complete`,
                {otp},getAuthHeaders());
    ...
    const handleCancel = async () => {
        ...
        setSubmitting(true);
        try {
            await axios.put(`${API_URL}/delivery/my_deliveries/${cancelModal}/cancel`,
                {reason: cancelReason}, getAuthHeaders());
